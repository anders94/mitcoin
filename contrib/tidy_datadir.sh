...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete mitcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no mitcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old mitcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected mitcoin 0.7 datadir."
    ;;
  3)
    echo "Detected mitcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected mitcoin 0.8 datadir."
    ;;
esac
