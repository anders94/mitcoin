...
*Setup instructions for a gitian build of mitcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the mitcoin
...
- [Building mitcoin](#building-mitcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-mitcoin-host/README.md).
...
Clone the git repositories for mitcoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mitcoin/mitcoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mitcoin repository
...
To build mitcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the mitcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mitcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mitcoin/mitcoin
...
For example:
```bash
URL=https://github.com/laanwj/mitcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mitcoin=${COMMIT} --url mitcoin=${URL} ../mitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mitcoin=${COMMIT} --url mitcoin=${URL} ../mitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mitcoin=${COMMIT} --url mitcoin=${URL} ../mitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mitcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mitcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mitcoin-osx-build.assert
```
...
[mitcoin/gitian.sigs](https://github.com/mitcoin/gitian.sigs/) repository, or if that's not possible create a pull
