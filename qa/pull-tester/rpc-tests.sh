...
export mitcoinCLI=${BUILDDIR}/qa/pull-tester/run-mitcoin-cli
export mitcoinD=${REAL_mitcoinD}
...
if [ "x${ENABLE_mitcoinD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and mitcoind must all be enabled"
fi
